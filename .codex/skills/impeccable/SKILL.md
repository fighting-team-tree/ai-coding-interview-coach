---
name: impeccable
description: 높은 디자인 완성도를 갖춘 개성 있고 프로덕션 수준의 프론트엔드 인터페이스를 만듭니다. 흔한 AI 스타일을 피한 창의적이고 정제된 코드를 생성합니다. 웹 컴포넌트, 페이지, 아티팩트, 포스터, 애플리케이션 제작을 요청하거나 다른 디자인 스킬이 프로젝트 맥락을 필요로 할 때 사용합니다. 'craft'는 설계 후 구현, 'teach'는 디자인 컨텍스트 설정, 'extract'는 재사용 가능한 컴포넌트와 토큰을 디자인 시스템으로 추출할 때 호출합니다.
version: 2.1.1
